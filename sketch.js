...
const PROBLEM2_X_OFFSET = 30; // 問題2 距離右側的偏移量（小）
...
function drawQuestionUI() {
  const inputBoxH = 40; // 輸入框高度
  const inputBoxW = 400; // 輸入框寬度
  ...
  if (!currentQuestion.hintDisplay) {
    ...
    stroke(255);
    strokeWeight(2);
    rect(questionBoxX - 120, questionBoxY - 80, 240, 50, 8);

    fill(255);
    textSize(14);
    textAlign(CENTER, CENTER);
    text(currentQuestion.text, questionBoxX, questionBoxY - 55);
  }
